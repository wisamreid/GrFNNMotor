...
global gds  MC
    ...
    if isfield(gds,'userfunction'),starter('start_userfunctions',handles,pos1);end
    %pos1 = starter('start_multipliers',handles,pos1);
    starter('in_start',handles);
%-------------------------------------------------------------------------    
function load_layout(handles)
global gds 
    if size(gds.numeric.HSN,1)<7
         gds.numeric.HSN{6,1} = 'user functions';
         gds.numeric.HSN{6,2} = 0;
         gds.numeric.HSN{7,1} = 'npoints';
         gds.numeric.HSN{7,2} = 0;
     end
     for i = 1:7
         if gds.numeric.HSN{i,2}==1
             gds.numeric.HSN{i,1} = upper(gds.numeric.HSN{i,1});
         elseif gds.numeric.HSN{i,2}==0
             gds.numeric.HSN{i,1} = lower(gds.numeric.HSN{i,1});
         end
        string{i,1} = gds.numeric.HSN{i,1};
    end    
    set(handles.layoutbox,'String',string);
%-------------------------------------------------------------------------    
function layoutbox(list,index_selected)
global gds 
    c = gds.numeric.HSN{index_selected,2};    
    gds.numeric.HSN{index_selected,2} = 1-c;    
%-------------------------------------------------------------------------    
function numeric1(handles)
global gds  MC homds
    ndim = size(gds.parameters,1);
%     gds.numeric = rmfield(gds.numeric,'HSN');
    if ~isfield(gds.numeric,'HSN')  || strcmp(gds.numeric.HSN{4,1},'multipliers') || strcmp(gds.numeric.HSN{4,1},'MULTIPLIERS')       
        gds.numeric.HSN = {'parameters' 1;'period' 1;'testfunctions' 0;'eigenvalues' 1;'current stepsize' 0};
    end
     if size(gds.numeric.HSN,1)<7
         gds.numeric.HSN{6,1} = 'user functions';
         gds.numeric.HSN{6,2} = 0;
         ...
         gds.numeric.HSN{8,1} = 'eps0';
         gds.numeric.HSN{8,2} = 1;
         gds.numeric.HSN{9,1} = 'eps1';
         gds.numeric.HSN{9,2} = 1;
    if isfield(gds,'userfunction')
        dimu = size(gds.userfunction,2);
    else dimu=0; 
    end
    s = 2;
    if gds.numeric.HSN{1,2}==1
        s = s+2*ndim+2;
    end
    if gds.numeric.HSN{2,2}==1
        s = s+4;
    end
    if gds.numeric.HSN{3,2}==1
        s = s+8+2;
    end  
    if gds.numeric.HSN{4,2}==1
        s = s+4*gds.dim+2;
    end  
    if gds.numeric.HSN{5,2}==1
        s = s+2;
    end 
    if gds.numeric.HSN{6,2}==1
        s = s+2*dimu+2;
    end
    if gds.numeric.HSN{7,2}==1
        s = s+2;
    end    
    if gds.numeric.HSN{8,2}==1
        s = s+2;
    end    
    if gds.numeric.HSN{9,2}==1
        s = s+2;
    end    
    slider_step(1) = 2/s;
    slider_step(2) = 2/s;
...
%                 label{k}  = sprintf('xout((0:homds.tps-1)*homds.nphase+%d,i)',plot2(1,k));    
                ...
                label{k}  = sprintf('ones(homds.tps,1)*xout(%d,i)',plot2(1,k)); 
...
global gds  MC
    ...
    if numsing(1)~=0 && isfield(MC.numeric_handles,'NCH')   
        label{num}=sprintf('set(MC.numeric_handles.NCH,''String'',num2str(hout(%d,i(end)),''%%0.8g''))',numsing(1)+dimu);       
        num = num + 1;
    end
...
global gds path_sys eds cds homds MC
     if ~isempty(varargin)
         file = fullfile(path_sys,gds.system,gds.diagram,strcat(gds.curve.new,'.mat'));
         if exist(file,'file')
             load(file);
         else
             errordlg('It is not possible to extend the current curve!','Error extend HSN');
             return 
         end  
         ...
         nextra = length(find(gds.extravec));
         if (n_par + nextra) ~= 4
         ...
         switch gds.point           
             case {'NCH '}  %case homoclinic starts from a non-central HSN
                 par = vertcat(gds.parameters{:,2});
                 [x0,v0] = init_NCH_HSN(systemhandle,x,v,s(num),par,gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol,gds.extravec,gds.T,gds.eps0,gds.eps1);
                 if isempty(x0),set(MC.mainwindow.mstatus,'String','Non-Central HSN');end
             case {'LC '} %case homoclinic starts from a LC
                 par = vertcat(gds.parameters{:,2});
                 [x0,v0] = init_LC_HSN(systemhandle,x,s(num),par,gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol,gds.extravec,gds.T,gds.eps0,gds.eps1);
             ...
             case {'HTHSN '}
                 ...
                 x0 = x(:,s(num).index);
                 v0 = v(:,s(num).index);                 
                 x0 = [x0(1:(gds.discretization.ntst*gds.discretization.ncol+1)*gds.dim);gds.x0];
                 [x0,v0] = init_HTHSN_HSN(systemhandle, x0, v0, s(num), par, gds.options.ActiveParams, gds.discretization.ntst, gds.discretization.ncol,gds.extravec,gds.T,gds.eps0,gds.eps1);     
             otherwise
                 par = vertcat(gds.parameters{:,2});
                 if ~strcmp(func2str(cds.curve),func2str(@homoclinicsaddlenode))
                     changeit = 1;
                 else 
                     changeit = 0;
                 end
                 [x0,v0] = init_HSN_HSN(systemhandle,x,v,s(num),par,gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol,gds.extravec,gds.T,gds.eps0,gds.eps1);
                 if changeit
                     gds.T = homds.T;
                 end
                  if strcmp(gds.curve.old,'tempadwgyk_cycle')
...
for k = 1
    ...
    switch k
    case 1
        string = 'Non-central homoclinic-to-saddle-node';
    otherwise
...
set(edit,'Callback','Callback');
user.pos = pose; set(edit,'Position',pose,'UserData',user);post = [2 j 18 1.8];
    pos = [2 j 18 1.8]; user.pos=pos;
    set(rad,'Position',pos,'UserData',user); guidata(handles.figuur,handles);
...
%--------------------------------------------------------------------------
function j = numeric_hsnpars(handles,j)
global gds MC ;
dim = size(gds.parameters,1);
color = [1 1 1];
j = j-2;
