function status = integ_plotConnecHom(t,y,flag,varargin)
...
global gds sys ud MC driver_window calculation_progress string_ConnecHom npoints saddle_point;
...
if isempty(flag) % odephas2(t,y) [v5 syntax] or odephas2(t,y,'')
    ...
    for l = 1:size(y,2)
        eps1(l) = norm(y(:,l)-saddle_point);
    end
    ud.t(oldi+1:newi) = t;
    ud.y(oldi+1:newi,:) = y.';
    ud.eps1(oldi+1:newi) = eps1;
    ud.i = newi;
    ...
    case 'init'                           % odephas2(tspan,y0,'init')
        ...
        string_ConnecHom = feval(gds.gui_load_draw);
        ud = [];
        ud.nr = 1;
        cols = length(y);
        ud.t = zeros(128,1);
        ...
        ud.t(1) = t(1);
        ud.y(1,:) = y(1:gds.dim).';
        ud.eps1(1) = norm(y(1:gds.dim) - saddle_point);        
       ...
       for i=1:gds.dim,x(i,1)=0;end;    
        if size(MC.D2,2)>0        
            ...
            for siz=1:size(MC.D2,2)
                ...
                if isempty(strmatch(char(gds.plot2(nr,1).type),string_ConnecHom,'exact'))|isempty(strmatch(char(gds.plot2(nr,2).type),string_ConnecHom,'exact'))
                    ud.p2=[inf inf];
                else
        ...
        if size(MC.D3,2)>0
            ...
            for siz=1:size(MC.D3,2)
                ...
                dat = get(MC.D3(siz),'UserData');
                ud.nr = dat.nr;nr=dat.nr;
                if isempty(strmatch(char(gds.plot3(nr,1).type),string_ConnecHom,'exact'))|isempty(strmatch(char(gds.plot3(nr,2).type),string_ConnecHom,'exact'))|isempty(strmatch(char(gds.plot3(nr,3).type),string_ConnecHom,'exact'))
                    ud.p3=[inf inf inf];
                else
        ...
        if ~isempty(MC.numeric_fig)         
            ...
            if isfield(MC.numeric_handles,'eps1')
                ud.label{num}='set(MC.numeric_handles.eps1,''String'',num2str(eps1(end,1)))';          
                num = num+1;
            end
