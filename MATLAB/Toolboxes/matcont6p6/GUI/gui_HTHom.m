...
if size(gds.numeric.HTHom,1)<7
    gds.numeric.HTHom{6,1} = 'user functions';    
    gds.numeric.HTHom{6,2} = 0;   
    gds.numeric.HTHom{7,1} = 'npoints';    
    gds.numeric.HTHom{7,2} = 0;    
end
for i = 1:7
    if gds.numeric.HTHom{i,2}==1    
        gds.numeric.HTHom{i,1} = upper(gds.numeric.HTHom{i,1});        
    elseif gds.numeric.HTHom{i,2}==0   
        gds.numeric.HTHom{i,1} = lower(gds.numeric.HTHom{i,1});       
    end
    string{i,1} = gds.numeric.HTHom{i,1};   
end
...
c = gds.numeric.HTHom{index_selected,2};    
gds.numeric.HTHom{index_selected,2} = 1-c; 
...
ndim = size(gds.parameters,1);
if ~isfield(gds.numeric,'HTHom') 
    gds.numeric.HTHom = {'parameters' 1;'period' 0;'testfunctions' 1;'multipliers' 0;'current stepsize' 0};    
end

if size(gds.numeric.HTHom,1)<6
    gds.numeric.HTHom{6,1} = 'user functions';    
    gds.numeric.HTHom{6,2} = 0;    
    gds.numeric.HTHom{7,1} = 'npoints';    
    gds.numeric.HTHom{7,2} = 0;    
end
gds.numeric.HTHom{8,1} = 'eps1';
gds.numeric.HTHom{8,2} = 1;    

for i = 1:size(gds.UParams,1)
    gds.numeric.HTHom{8+i,1} = strcat('UParam',i);   
    gds.numeric.HTHom{8+i,2} = 1;   
end
val = 8+size(gds.UParams,1);
for i = 1:size(gds.SParams,1)
    gds.numeric.HTHom{val+i,1} = strcat('SParam',i);   
    gds.numeric.HTHom{val+i,2} = 1;    
end
...
if gds.numeric.HTHom{1,2}==1
    s = s+2*ndim+2;   
end
if gds.numeric.HTHom{2,2}==1
    s = s+4;    
end
if gds.numeric.HTHom{3,2}==1
    s = s+2*dims+2;   
end
if gds.numeric.HTHom{4,2}==1
    s = s+4*gds.dim+2;   
end
if gds.numeric.HTHom{5,2}==1
    s = s+2;    
end
if gds.numeric.HTHom{6,2}==1
    s = s+2*dimu+2;   
end
if gds.numeric.HTHom{7,2}==1
    s = s+2;   
end
if gds.numeric.HTHom{8,2}==1
    s = s+2;    
end
for k = 1:size(gds.UParams,1)
    if gds.numeric.HTHom{8+k,2}==1   
        s = s+2;   
    end
end
val = 8+size(gds.UParams,1);
for k = 1:size(gds.SParams,1)
    if gds.numeric.HTHom{val+k,2}==1   
        s = s+2;        
    end
end
...
if gds.numeric.HTHom{8,2}==1
    j = numeric_HTHomeps1(handles,j);   
end
if gds.numeric.HTHom{9,2}==1
    numeric_HTHompars(handles,j);        
end
...
for j = 1:len
    switch pl(j).type    
            ...
            if HTHomds.index == 1
                ...
                for k = 1:size(gds.options.ActiveSParams,2)
                    if pl(j).val==gds.options.ActiveSParams(1,k)
                        e(1,j) = HTHomds.ncoords+size(gds.options.ActiveUParams,2)+k;
                        val = 1;
                    end
                end
                if val ==0
        ...
        case 'eps1'
            if HTHomds.index == 1
                e(1,j) = HTHomds.ncoords+size(gds.options.ActiveUParams,2)+size(gds.options.ActiveSParams,2)+1;
            else
                e(1,j) = HTHomds.PeriodIdx+3;
            end
...
for k = 1:len
    ...
    if plot2(1,k)<0   
        if (plot2(1,k)>=-ndim)
            pl2  = -plot2(1,k);        
            label{k}  = sprintf('gds.parameters{%d,2}*ones(HTHomds.tps,p)',pl2);          
            lab{k}  = sprintf('gds.parameters{%d,2}',pl2);                      
        elseif (plot2(1,k)<-ndim) && (plot2(1,k)>=-ndim-size(gds.UParams,1))
            pl2  = -plot2(1,k)-ndim;            
            label{k}  = sprintf('gds.UParams{%d,2}*ones(HTHomds.tps,p)',pl2);              
        ...
        elseif (plot2(1,k)<-ndim-size(gds.UParams,1)) && (plot2(1,k)>=-ndim-size(gds.UParams,1)-size(gds.SParams,1))        
            ...
            label{k}  = sprintf('gds.SParams{%d,2}*ones(HTHomds.tps,p)',pl2);              
            lab{k}  = sprintf('gds.SParams{%d,2}',pl2);           
        end
    end    
    if plot2(1,k)==0    
        label{k}  = 'gds.time{1,2}*ones(HTHomds.tps,p)';        
        lab{k} = 'gds.time{1,2}';        
    end
    ...
    if plot2(1,k) < -(ndim+size(gds.UParams,1)+size(gds.SParams,1)) && plot2(1,k) >-10000*ndim%case eigenvalues                   
        if mod(plot2(1,k)+ndim+size(gds.UParams,1)+size(gds.SParams,1),2)==0        
            label{k}  = sprintf('angle(fout(HTHomds.ntst+1+%d,i))',(-plot2(1,k)-(ndim+size(gds.UParams,1)+size(gds.SParams,1)))/2);            
            lab{k} = sprintf('angle(fout(HTHomds.ntst+1+%d,i))',(-plot2(1,k)-(ndim+size(gds.UParams,1)+size(gds.SParams,1)))/2);            
        else %Re        
            label{k} = sprintf('abs(fout(HTHomds.ntst+1+%d,i))',(-plot2(1,k)-(ndim+size(gds.UParams,1)+size(gds.SParams,1))+1)/2);            
            lab{k} = sprintf('abs(fout(HTHomds.ntst+1+%d,i))',(-plot2(1,k)-(ndim+size(gds.UParams,1)+size(gds.SParams,1))+1)/2);                    
        end
    ...
    if strcmp(label{k},'empty')==1    
        ...
        if plot2(1,k) <= gds.dim %case coordinate        
            label{k}  = sprintf('xout((0:HTHomds.tps-1)*HTHomds.nphase+%d,i)',plot2(1,k));            
            lab{k}  = sprintf('xout((0:HTHomds.tps-1)*HTHomds.nphase+%d,i)]',plot2(1,k));                      
        else
            label{k} = sprintf('ones(HTHomds.tps,1)*xout(%d,i)',plot2(1,k));            
...
if size(gds.SParams,1)~=0
    if (HTHomds.index == 0) || (HTHomds.index == 1) || (HTHomds.index == 2)            
...
if (HTHomds.index == 3) || (size(gds.SParams,1) == 0)
    ...
    case 2    
        ...
        for k = 1:2
            if plot2(1,k)<0 
                if plot2(1,k)>=-ndim
                    pl2 = -plot2(1,k);
                    plo2{k} = gds.parameters{pl2,2}*ones(HTHomds.tps,p); 
                elseif (plot2(1,k)<-ndim) && (plot2(1,k)>=-size(gds.UParams,1)-ndim)
                    pl2 = -plot2(1,k)-ndim;
                    plo2{k} = gds.UParams{pl2,2}*ones(HTHomds.tps,p);                  
                elseif (plot2(1,k)<-size(gds.UParams,1)-ndim) && (plot2(1,k)>=-size(gds.UParams,1)-size(gds.SParams,1)-ndim)
                    ...
                    plo2{k} = gds.SParams{pl2,2}*ones(HTHomds.tps,p); 
                end   
            end
            if (plot2(1,k)==0)
                plo2{k} = gds.time{1,2}*ones(HTHomds.tps,p);
            end
            ...
            if plot2(1,k) < -(ndim+size(gds.UParams,1)+size(gds.SParams,1)) && plot2(1,k)>-10000*ndim
                if mod(plot2(1,k)+size(gds.UParams,1)+size(gds.SParams,1)+ndim,2)==1
                    plo2{k} = abs(f(HTHomds.ntst+1+(-plot2(1,k)-(size(gds.UParams,1)+size(gds.SParams,1)+ndim)+1)/2,:));
                else %modulus
                    plo2{k} = angle(f(HTHomds.ntst+1+(-plot2(1,k)-(size(gds.UParams,1)+size(gds.SParams,1)+ndim)+1)/2,:));
                end
            elseif plot2(1,k) <= -100000*ndim % case Userfunction
                plo2{k}  = ones(HTHomds.tps,1)*h(2-plot2(1,k)-100000*ndim,:);      
            end
                                                  
            if strcmp(plo2{k},'empt')==1
                if plot2(1,k) <= gds.dim 
                    dimplot = dimplot+1;
                    plo2{k} = x((0:HTHomds.tps-1)*HTHomds.nphase+plot2(1,k),:);
                else
                    plo2{k} = ones(HTHomds.tps,1)*x(plot2(1,k),:);
                end
            end            
        end
        if dimplot == 0
        ...
        for k = 1:3
            if plo(1,k)<0
                if plo(1,k)>=-ndim %case non active parameter
                    pl2 = -plo(1,k);
                    plo3{k} = gds.parameters{pl2,2}*ones(HTHomds.tps,p); 
                elseif (plo(1,k)<-ndim) && (plo(1,k)>=-size(gds.UParams,1)-ndim)
                    pl2 = -plo(1,k)-ndim;
                    plo3{k} = gds.UParams{pl2,2}*ones(HTHomds.tps,p);            
                elseif (plo(1,k)<-size(gds.UParams,1)-ndim) && (plo(1,k)>=-size(gds.UParams,1)-size(gds.SParams,1)-ndim)
                    ...
                    plo3{k} = gds.SParams{pl2,2}*ones(HTHomds.tps,p);                    
                elseif plo(1,k) <= -100000*ndim % case Userfunction
                    plo3{k}  = ones(HTHomds.tps,1)*h(2-plo(1,k)-100000*ndim,:);        
                elseif plo(1,k) < -(ndim+size(gds.UParams,1)+size(gds.SParams,1)) % Case multipliers
                    if mod(plo(1,k)+size(gds.UParams,1)+size(gds.SParams,1)+ndim,2)==1
                        plo3{k} = ones(HTHomds.tps,1)*abs(f((-plo(1,k)-(size(gds.UParams,1)+size(gds.SParams,1)+ndim)+1)/2,:));
                    else %Mod
                        plo3{k} = ones(HTHomds.tps,1)*angle(f((-plo(1,k)-(size(gds.UParams,1)+size(gds.SParams,1)+ndim))/2,:));
                    end
                end
            end
            if (plo(1,k)==0) %case time
                plo3{k} = gds.time{1,2}*ones(HTHomds.tps,p);
            end
            if strcmp(plo3{k},'empt')==1
                if plo(1,k)<=gds.dim       %case coordinate
                    dimplot = dimplot+1;
                    plo3{k}=x((0:HTHomds.tps-1)*HTHomds.nphase+plo(1,k),:);
                else
                    plo3{k}=x(plo(1,k)*ones(HTHomds.tps,1),:);  
                end
            end            
        end
        if dimplot == 0
...
if ~isempty(MC.numeric_fig) %numeric window is open
    if HTHomds.index == 1
        ...
        for k = 1:length(aup)    
            j = aup(k);                 
            UParams(j) = xout(HTHomds.ncoords+k,i(end));                        
        end
        ...
        for k = 1:length(asp)    
            j = asp(k);                 
            SParams(j) = xout(HTHomds.ncoords+length(aup)+k,i(end));                        
        end

        eps1 = xout(HTHomds.ncoords+length(aup)+length(asp)+1,i(end));
        T = gds.T;   
        eps0 = gds.eps0;
    ...
    elseif HTHomds.index == 2
        ...
        for k = 1:length(ap)    
            j = ap(k);                 
            parameters(j) = xout(HTHomds.nphase*(HTHomds.tps+1)+k,i(end));                        
        end
        ...
        for k = 1:length(asp)    
            j = asp(k);                 
            SParams(j) = xout(HTHomds.nphase*(HTHomds.tps+1)+1+k,i(end));                        
        end                
           
        T = gds.T;    
        eps0 = gds.eps0;    
        eps1 = xout(HTHomds.nphase*(HTHomds.tps+1)+3,i(end));
        ...
        newind = HTHomds.nphase*(HTHomds.tps+1)+length(ap)+1;
...
elseif HTHomds.index == 2
    ...
    if ~isfield(HTHomds,'TestTolerance') || isempty(HTHomds.TestTolerance)
        HTHomds.TestTolerance = 1e-5;
    end
...
global gds path_sys cds HTHomds MC
...
if ~isempty(varargin)
    ...
    if exist(file,'file')    
        ...
        errordlg('It is not possible to extend the current curve!','Error extend HTHom');        
        return         
    end
    ...
    if number < (size(gds.SParams,1)-1)
        ...
        if HTHomds.index == 0 %ConnA_ConnB       
            ...
            [x0,v0] = init_HTHom_HTHom(systemhandle,x,v,s,vertcat(gds.parameters{:,2}), gds.options.ActiveParams,parU,gds.options.ActiveUParams,parS,gds.options.ActiveSParams,gds.discretization.ntst,gds.discretization.ncol,gds.T,gds.eps1,gds.eps1tol);                                    
        elseif HTHomds.index == 1 %ConnB_ConnB             
            x0 = x(:,s(num).index);%enkel die éne kolom is belangrijk!                                  
            x0 = [x0(1:(HTHomds.ntst*HTHomds.ncol+1)*gds.dim);gds.x0];
            v0 = v(:,s(num).index);            
            parU = vertcat(gds.UParams{:,2});            
            parS = vertcat(gds.SParams{:,2});  
            [x0,v0] = init_HTHom_HTHom(systemhandle, x0, v0, s(num),HTHomds.P0,gds.options.ActiveParams, parU, gds.options.ActiveUParams, parS, gds.options.ActiveSParams, gds.discretization.ntst,gds.discretization.ncol,gds.T,gds.eps1,gds.eps1tol);            
        end
    ...
    elseif number == (size(gds.SParams,1)-1)                    
        ...
        if gds.SParams{gds.options.ActiveSParams(1),2} == 0                
            error('A zero SParam should not be denoted as free');
        end
        ...
        n_par = size(gds.options.ActiveParams,2);
        if n_par ~= 1
            error('1 free system parameter is needed');
        end
        ...
        if HTHomds.index == 0 %ConnA_ConnC
            ...
            parS = vertcat(gds.SParams{:,2});
            x = [x;gds.x0];  
            [x0,v0] = init_HTHom_HTHom(systemhandle, x, v, s, HTHomds.P0, gds.options.ActiveParams, [], [], parS, gds.options.ActiveSParams, gds.discretization.ntst, gds.discretization.ncol,HTHomds.T,HTHomds.eps1,gds.eps1tol);
        elseif HTHomds.index == 1 %ConnB_ConnC     
            HTHomds.index = 2;
            x0 = x(:,s(num).index);%enkel die éne kolom is belangrijk!     
            v0 = v(:,s(num).index);    
            x0 = [x0(1:(HTHomds.ntst*HTHomds.ncol+1)*gds.dim);gds.x0];%opgelet! gds.x0 ipv HTHomds.x0
            par = vertcat(gds.parameters{:,2});          
            parS = vertcat(gds.SParams{:,2});           
            [x0,v0] = init_HTHom_HTHom(systemhandle, x0, v0, s(num), par, gds.options.ActiveParams, [], [], parS, gds.options.ActiveSParams, gds.discretization.ntst, gds.discretization.ncol,gds.T,gds.eps1,gds.eps1tol);      
        ...
        if HTHomds.index == 2 %ConnC_ConnD
            ...
            par = vertcat(gds.parameters{:,2});            
            gds.T = gds.period/2;            
            [x0,v0] = init_HTHom_HTHom(systemhandle, x0, v0, s(num), par, gds.options.ActiveParams,[],[],[],[],gds.discretization.ntst,gds.discretization.ncol,gds.T,gds.eps1,gds.eps1tol);
        else %ConnD_ConnD
            x0 = x(:,s(num).index);            
            v0 = v(:,s(num).index);            
            x0 = [x0(1:(HTHomds.ntst*HTHomds.ncol+1)*gds.dim);gds.x0];
    ...
    [x,v,s,h,f] = cont(@homotopysaddle,x0,v0,gds.options);    
end
...
if ~isempty(x)
    save(file,'x','v','s','h','f','cds','HTHomds','ctype','point','num');    
end    
...
for i = 1:dim_nneg
    if abs(gds.SParams{i,2}) < HTHomds.TestTolerance
        gds.SParams{i,2} = 0;
    end
end
...
for i = 1:dim_nneg
    ...
    tag2 = strcat('edit',num2str(gds.dim+size(gds.parameters,1)+dim_npos+i));
    user.num = num2str(i);
    edit = uicontrol(handles.figuur,'Style','edit','HorizontalAlignment','left','String',string,'Tag',tag2,'Backgroundcolor',color,'units','characters','fontname','FixedWidth','fontsize',12);
...
if ~isfield(HTHomds,'index') || isempty(HTHomds.index)
    ...
    if ~isempty(gds.SParams)
        if (HTHomds.index == 0 || HTHomds.index == 1 || HTHomds.index == 2)    
            ...
            if ~isfield(HTHomds,'TestTolerance') || isempty(HTHomds.TestTolerance)    
                ...
                vec = zeros(size(sp))+HTHomds.TestTolerance;        
            end
...
%--------------------------------------------------------------------------
function j = numeric_HTHompars(handles,j)
global gds
color = [1 1 1];
j = j-2;
...
%--------------------------------------------------------------------------
function j = numeric_HTHomeps1(handles,j)
global gds
color = [1 1 1];
j = j-2;
