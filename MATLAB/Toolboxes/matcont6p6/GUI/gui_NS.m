...
global gds lds MC cds
    ...
    if isfield(MC.numeric_handles,'period')
        label{num}='set(MC.numeric_handles.period,''String'',num2str(xout(end-3,i(end)),''%0.8g''))';
        num = num +1;
    end
    ...
    if numsing(1)~=0 && isfield(MC.numeric_handles,'R1')   
        label{num}=sprintf('set(MC.numeric_handles.R1,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(1)+dimu);       
        num = num + 1;
    end
    if numsing(2)~=0 && isfield(MC.numeric_handles,'R2') 
        label{num}=sprintf('set(MC.numeric_handles.R2,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(2)+dimu);
        num = num+1;
    end
    if numsing(3)~=0 && isfield(MC.numeric_handles,'R3') 
        label{num} = sprintf('set(MC.numeric_handles.R3,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(3)+dimu);
        num = num+1;
    end
    if numsing(4)~=0 && isfield(MC.numeric_handles,'R4') 
        label{num} = sprintf('set(MC.numeric_handles.R4,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(4)+dimu);
        num = num+1;
    end
    if numsing(5)~=0 && isfield(MC.numeric_handles,'LPNS') 
        label{num} = sprintf('set(MC.numeric_handles.LPNS,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(5)+dimu);
        num = num+1;
    end
    if numsing(6)~=0 && isfield(MC.numeric_handles,'CH') 
        label{num} = sprintf('set(MC.numeric_handles.CH,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(6)+dimu);
        num = num+1;
    end
    if numsing(7)~=0 && isfield(MC.numeric_handles,'PDNS') 
        label{num} = sprintf('set(MC.numeric_handles.PDNS,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(7)+dimu);
        num = num+1;
    end
    if numsing(8)~=0 && isfield(MC.numeric_handles,'NSNS') 
        label{num} = sprintf('set(MC.numeric_handles.NSNS,''String'',num2str(hout(%d,i(end)),''%%.8g''))',numsing(8)+dimu);
        num = num+1;
    end
    ...
    case 3 %3D
        ...
        if strcmp(varargin{4},'redraw')              
            ...
            if ~strcmp(plo3s{1},'empt')&&~strcmp(plo3s{2},'empt')&&~strcmp(plo3s{3},'empt')
                line(plo3s{1},plo3s{2},plo3s{3},'linestyle','none','Marker','.','MarkerEdgeColor','r');
            end 
            if ~isempty(s)
...
global gds lds
    ...
    if len>0
        for i=1:len
            ...
            gds.parameters{j,2} = x(end-3+i,index);
        end
    end
...
global gds
    if size(gds.options.IgnoreSingularity,2)==8
...
global gds path_sys cds lds 
     if ~isempty(varargin)
        ...
        case 'ZH '
            ...
            [x0,v0] = init_ZH_NS(systemhandle,x,par,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol,gds.eps);            
        case 'HH ' 
            if ~isfield(gds,'eps') || isempty(gds.eps)
                gds.eps = 0.01;
            end
            if gds.whichNS == 1
                [x0,v0] = init_HH_NS1(systemhandle,x,par,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol,gds.eps);         
            else
                [x0,v0] = init_HH_NS2(systemhandle,x,par,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol,gds.eps);         
            end
        case 'LPNS '
            [x0,v0] = init_LPNS_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        case 'PDNS '
            [x0,v0] = init_PDNS_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        case 'CH '
            [x0,v0] = init_CH_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        case 'R1 '
            [x0,v0] = init_R1_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        case 'R2 '
            [x0,v0] = init_R2_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        case 'R3 '
            [x0,v0] = init_R3_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        case 'R4 '
            [x0,v0] = init_R4_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        otherwise
            [x0,v0] = init_NS_NS(systemhandle,x,s(num),gds.options.ActiveParams,gds.discretization.ntst,gds.discretization.ncol);
        end
        if isempty(x0)
            return
        end       
        [x,v,s,h,f] = cont(@neimarksacker,x0,v0,gds.options);      
    end
...
for k = 1:8
    ...
    case 3
        string = '1:3 resonance';       
    case 4
        string = '1:4 resonance';
    case 5 %LPNS
        if (gds.dim < 4)
            continue;
        end
