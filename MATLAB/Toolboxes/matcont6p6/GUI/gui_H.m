...
global gds MC
    ...
    pos1 = starter('start_parameters',handles,pos1);%make place for parameters
...
global gds path_sys cds hds cds MC
    ndim = size(gds.parameters,1);        
    if size(gds.numeric.EP,1)<7
        gds.numeric.EP{6,1} = 'user functions';
        gds.numeric.EP{6,2} = 0;
...
global gds MC
    ...
    if numsing(3)~=0 && isfield(MC.numeric_handles,'DH') 
        label{num} = sprintf('set(MC.numeric_handles.DH,''String'',num2str(hout(%d,i(end))))',numsing(3)+dimu);
        num = num+1;
    end
    if numsing(4)~=0 && isfield(MC.numeric_handles,'GH') 
        label{num} = sprintf('set(MC.numeric_handles.GH,''String'',num2str(hout(%d,i(end)),''%%0.8g''))',numsing(4)+dimu+control);
        num = num+1;
    end
...
global gds path_sys hds cds MC
    ndim = size(gds.parameters,1);   
 ...
 global gds path_sys cds hds MC
    ndim = size(gds.parameters,1);        
    npoints=i(end);
    x1 = xout(:,i); 
...
global gds hds 
    ...
    ndim = size(hds.P0,1);
    for i = 1:ndim
        gds.parameters{i,2} = hds.P0(i,1);
    end    
    for i = 1:2
        j = hds.ActiveParams(1,i);
        gds.parameters{j,2} = x((gds.dim)+i,index);            
    end
    if strcmp(string,'save')
        num = varargin{1};
        save(file,'x','v','s','h','f','num','hds','cds','ctype','point');
    end
%---------------------------------------------------------------------------
...
global gds path_sys cds hds        
     if ~isempty(varargin)
        ...
        switch gds.point
        case 'GH '
            [x0,v0] = init_GH_H(systemhandle, x0, par, gds.options.ActiveParams);
        case 'HH '
            [x0,v0] = init_HH_H(systemhandle, x0, par, gds.options.ActiveParams);
        case 'ZH '
            [x0,v0] = init_ZH_H(systemhandle, x0, par, gds.options.ActiveParams);
        case 'BT '
            [x0,v0] = init_BT_H(systemhandle, x0, par, gds.options.ActiveParams);
        otherwise
            [x0,v0] = init_H_H(systemhandle, x0, par, gds.options.ActiveParams);
        end
        if isempty(x0)
            return
        end       
        [x,v,s,h,f] = cont(@hopf, x0, v0, gds.options); 
    end
    ...
    if ~isempty(x)
        save(file,'x','v','s','h','f','cds','hds','ctype','point','num');            
    end
