function status = integ_plotConnecHet(t,y,flag,varargin)
...
global gds sys ud MC driver_window calculation_progress string_ConnecHet npoints;
...
if isempty(flag) % odephas2(t,y) [v5 syntax] or odephas2(t,y,'')
    ...
    for l = 1:size(y,2)
        eps1(l) = norm(y(:,l)-vertcat(gds.x1{:,2}));
    end
    ud.t(oldi+1:newi) = t;
    ud.y(oldi+1:newi,:) = y.';
    ...
    case 'init'                           % odephas2(tspan,y0,'init')
        ...
        string_ConnecHet = feval(gds.gui_load_draw);
        ud = [];
        ud.nr = 1;
        cols = length(y);
        ud.t = zeros(128,1);
        ...
        ud.t(1) = t(1);
        ud.y(1,:) = y(1:gds.dim).';
        ud.eps1(1) = norm(y(1:gds.dim) - vertcat(gds.x1{:,2}));        
       ...
       for i=1:gds.dim,x(i,1)=0;end;    
        if size(MC.D2,2)>0        
            ...
            for siz=1:size(MC.D2,2)
                ...
                if isempty(strmatch(char(gds.plot2(nr,1).type),string_ConnecHet,'exact'))|isempty(strmatch(char(gds.plot2(nr,2).type),string_ConnecHet,'exact'))
                    ud.p2=[inf inf];
                else
        ...
        if size(MC.D3,2)>0
            ...
            for siz=1:size(MC.D3,2)
                ...
                dat = get(MC.D3(siz),'UserData');
                ud.nr = dat.nr;nr=dat.nr;
                if isempty(strmatch(char(gds.plot3(nr,1).type),string_ConnecHet,'exact'))|isempty(strmatch(char(gds.plot3(nr,2).type),string_ConnecHet,'exact'))|isempty(strmatch(char(gds.plot3(nr,3).type),string_ConnecHet,'exact'))
                    ud.p3=[inf inf inf];
                else
