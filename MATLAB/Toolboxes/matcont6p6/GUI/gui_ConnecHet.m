...
pos1 = start_coordinatesx0x1(handles,pos1);
...
for i=1:3
    if gds.numeric.ConnecHet{i,2}==1    
        gds.numeric.ConnecHet{i,1}=upper(gds.numeric.ConnecHet{i,1});        
    elseif gds.numeric.ConnecHet{i,2}==0    
        gds.numeric.ConnecHet{i,1}=lower(gds.numeric.ConnecHet{i,1});        
    end
    string{i,1}=gds.numeric.ConnecHet{i,1};   
end
...
c = gds.numeric.ConnecHet{index_selected,2};    
gds.numeric.ConnecHet{index_selected,2} = 1-c;    
%-------------------------------------------------------------------------    
function numeric1(handles)
global gds
ndim=size(gds.parameters,1);

if ~isfield(gds.numeric,'ConnecHet') 
    gds.numeric.ConnecHet = {'time' 1;'coordinates' 1;'eps1' 1;'parameters' 0'};    
end

s = 2;
if gds.numeric.ConnecHet{1,2}==1
    s = s+3;   
end
if gds.numeric.ConnecHet{2,2}==1
    s = s+2*gds.dim+2;   
end
if gds.numeric.ConnecHet{3,2}==1
    s = s+40;   
end
if gds.numeric.ConnecHet{4,2}==1
    s = s+2*ndim+2;    
end
s = s+2;
slider_step(1) = 2/s;
...
gds.UParams = HTHetds.UParams;
gds.SParams = HTHetds.SParams;
gds.extravec = HTHetds.extravec;
...
global gds MC path_sys t dim_npos dim_nneg QS
...
if ~(norm(func)<1e-1)
    errordlg('x0 is not an equilibrium','Bad startpoint');
    return
end
% is x_1 an equilibrium?
func = feval(func_handles{2}, 0, vertcat(gds.x1{:,2}), p{:});
if ~(norm(func)<1e-1)
    errordlg('x1 is not an equilibrium','Bad startpoint');
    return
end
...
start_point = vertcat(gds.x0{:,2}) + gds.eps0*(gds.c{1,2}*QU(:,1)+gds.c{2,2}*QU(:,2));
...
try
    ...
    if size(MC.D2,2)>0||size(MC.D3,2)>0||~isempty(MC.numeric_fig)    
        gds.integrator.options = odeset(gds.integrator.options,'OutputFcn',@integ_plotConnecHet);        
    else
    ...
    for k = 1:size(y,1)    
        y1(k,1:gds.dim) = y(k,:);     
        y1(k,gds.dim+1) = norm(vertcat(gds.x1{:,2})'-y(k,:));        
    end
    ...
    A = feval(jaco, 0,vertcat(gds.x1{:,2}), p{:});
    D = eig(A); %dimensie van D : dim maal 1
    dim_nneg = sum(real(D) < 0);
    [QS, eigvl1] = computeBaseConnecHet(A,1);%n maal n matrix
...
%-----------------------------------------------------------------------------------
function  [Q0,sortedevls] = computeBaseConnecHet(A0,flag)
%unstable
if flag == 0       
    [evc,evl] = eig(A0);
...
stat = uicontrol(handles.figuur,'Style','pushbutton','String','Select ConnectionHet','Tag','select_cycleConnecHet','units','characters','fontname','FixedWidth','fontsize',12);
set(stat,'Position',pos,'UserData',user,'Callback','selectcycleConnecHet');
...
%--------------------------------------------------------------------------
function j = start_coordinatesx0x1(handles,j)
...
for i = 1:gds.dim
    ...
    gds.x1{i,1} = strcat('x1_',gds.coordinates{i,1});        
    gds.x0{i,2} = 0;
    gds.x1{i,2} = 0;
end
...
for i = 1:(gds.dim)
    if strcmp(gds.coordinates{i,1},'')
        ...
        string = num2str(gds.coordinates{i,2},'%0.8g');
        gds.x1{i,2} = gds.coordinates{i,2};
    end
    tag1 = strcat('text',num2str(2*gds.dim+ndim+i));
    tag2 = strcat('edit',num2str(2*gds.dim+ndim+i));
    edit = uicontrol(handles.figuur,'Style','edit','HorizontalAlignment','left','String',string,'Tag',tag2,'BackGroundColor',color,'units','characters','fontname','FixedWidth','fontsize',12);
    set(edit,'Callback','Callback');
    j = j-2;
    pos = [2 j 18 1.8];
    user.pos = pos;
    stat = uicontrol(handles.figuur,'Style','text','HorizontalAlignment','left','String',gds.x1{i,1},'Tag',tag1,'BackGroundColor',color,'units','characters','fontname','FixedWidth','fontsize',12);
    set(stat,'Position',pos,'UserData',user);
    pos = [20 j 25 1.8];user.pos=pos;
    set(edit,'Position',pos,'UserData',user);
end
