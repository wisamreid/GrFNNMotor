...
set(fig,'Color',get(0,'DefaultUicontrolBackgroundColor'));
   ...
   if strcmp(arg,'systems')||((nargin ==1)&&strcmp(varargin{1},'new'))
   ...
   if (exist('sym')==2)
    ...
    % force a resize, this ensures a triger of the ComponentResizeCallback
...
gds.equations=deblank(get(handles.sys,'String'));
try
...
for i=1:gds.dim
    if strcmp(upper(gds.coordinates{i}),'C')
        erdlg=sprintf('%s can not be used as a coordinate!',gds.coordinates{i});
        errordlg(erdlg,'wrong coordinate','modal');
        % don't clear, let the user clear C away, so reset focus to textbox
        uicontrol(handles.coordinates);
        return
    end
end
...
for i=1:ndim
    if strcmp(gds.parameters{i},'kmrgd')||strcmp(upper(gds.parameters{i}),'C')
        erdlg=sprintf('%s can not be used as a parameter!',gds.parameters{i});
        errordlg(erdlg,'wrong parameter','modal');
        % don't clear, let the user clear kmrgd, so reset focus to textbox
        uicontrol(handles.parameters);
        return
    end
end
