...
global gds 
    ...
    if gds.numeric.EP{3,2}==1
        if isfield(gds,'BranchParams'),bp=size(gds.BranchParams,2);else bp=0;end
        s = s+3*2+bp*2+2;
    end
...
global gds path_sys MC
    ...
    if numsing(1)~=0 && isfield(MC.numeric_handles,'BT')     
        label{num}=sprintf('set(MC.numeric_handles.BT,''String'',num2str(hout(%d,i(end)),''%%0.8g''))',numsing(1)+dimu);
        num = num + 1;
    end
    if numsing(2)~=0 && isfield(MC.numeric_handles,'ZH') 
        label{num}=sprintf('set(MC.numeric_handles.ZH,''String'',num2str(hout(%d,i(end)),''%%0.8g''))',numsing(2)+dimu);
        num = num+1;
    end
    if numsing(3)~=0 && isfield(MC.numeric_handles,'CP') 
        label{num} = sprintf('set(MC.numeric_handles.CP,''String'',num2str(hout(%d,i(end)),''%%0.8g''))',numsing(3)+dimu);
        num = num+1;
    end
    ...
    for ind =1:bp
        if numsing(ind+3)~=0 && isfield(MC.numeric_handles,strcat('BP',num2str(ind))) 
            label{num} = sprintf('set(MC.numeric_handles.BP%d,''String'',num2str(hout(%d,i(end)),''%%0.8g''))',ind,numsing(ind+3)+dimu);
            num = num+1;
        end
    end
...
global gds lpds 
    ...
    ndim = size(lpds.P0,1);
    for i = 1:ndim
        gds.parameters{i,2} = lpds.P0(i,1);
    end    
    if isempty(lpds.BranchParams)
        gds.BranchParams=[];
    else
        gds.BranchParams = lpds.BranchParams;
    end
    for i = 1:2
        j = lpds.ActiveParams(1,i);
        gds.parameters{j,2} = x((gds.dim)+i,index);
    end
    if strcmp(string,'save')
        num = varargin{1};
        save(file,'x','v','s','h','f','num','lpds','cds','ctype','point');
    end
%--------------------------------------------------------------------------- 
function singularities
global gds
    if isfield(gds,'BranchParams')&& size(gds.BranchParams,2)>0
        sbp = 1; else sbp = 0;end
    if size(gds.options.IgnoreSingularity,2)==(3+sbp)
...
global gds path_sys cds lpds
     if ~isempty(varargin)
        ...
        switch gds.point
        case 'BP '
            [x0,v0] = init_BP_LP(systemhandle, x0, par, gds.options.ActiveParams);
        case 'CP '
            [x0,v0] = init_CP_LP(systemhandle, x0, par, gds.options.ActiveParams);
        case 'ZH '
            [x0,v0] = init_ZH_LP(systemhandle, x0, par, gds.options.ActiveParams);
        case 'BT '
            [x0,v0] = init_BT_LP(systemhandle, x0, par, gds.options.ActiveParams);
        otherwise
            [x0,v0] = init_LP_LP(systemhandle,x0,par,gds.options.ActiveParams,bp); 
        end
        if isempty(x0)
            return
        end       
        [x,v,s,h,f] = cont(@limitpoint,x0,[],gds.options);      
    end
    ...
    if ~isempty(x)
        save(file,'x','v','s','h','f','cds','lpds','ctype','point','num');          
    end
