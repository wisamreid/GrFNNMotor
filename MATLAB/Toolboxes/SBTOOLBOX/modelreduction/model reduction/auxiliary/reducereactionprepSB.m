function [output] = reducereactionprep(output,reaction)
% reducereactionprep: This function transforms the reaction rate in the
% following form:
%                       a' * c = v
% 
...
% USAGE:
% ======
% output = reducereactionprep(model,reaction)
%
...
for k = 1:length(output.reaction_orig.variables),
    ...
    indexdata = strmatch(var,output.reference.data,'exact');
    if ~isempty(indexdata),
        variablesn(:,k) = output.reference.datavalues(:,indexdata);
    end
end
% get reaction values
indexdata = strmatch(output.reaction,output.reference.data,'exact');
...
for k = 1:length(output.reaction_trans.A),
    Aevalk = formula2vec(output.reaction_trans.A{k});
    An(:,k) = eval(Aevalk);
end
%%%%%%%%%%%%%%%%%%%%%%%
% Determine bn
