...
% options: if left out, default options are used. Otherwise the options can
%       be defined as:
%       
%       options = [plotTypeFlag];
%       options = [plotTypeFlag, visualizeFlag];
%       options = [plotTypeFlag, visualizeFlag, omegaRange, omegaRangePoints, ...
%                  bisectionThreshold, criticalLocusTolerance, criticalLocusNrPoints];
%       plotTypeFlag: =0: linear y-axis, =1: logarithmic y-axis
%       visualizeFlag: =0: nothing, =1: setting this flag to 1 allows to monitor the
%                   continuation of the critical locus and to see if the 
%                   values for criticalLocusTolerance and criticalLocusNrPoints
%                   are set acceptably.
%       omegaRange: to determine the critical frequency omega0 the function
...
%                   system.
%       omegaRangePoints: the above range is discretized in so many steps,
%                   allowing to find a lower and upper bound for omega0
%       bisectionThreshold: omega0 is then determined by bisection until
%                   the bounds have a distance less than this option value
%       criticalLocusTolerance: each determined perturbation Delta_ij is
...
%       criticalLocusNrPoints: the number of points used for the
...
elseif nargin == 3,
    ...
    elseif length(options) == 1,
        plotTypeFlag = options;
        visualizeFlag = 0;
        options = [0 0 0 0 0];
    elseif length(options) == 2,
        plotTypeFlag = options(1);
        visualizeFlag = options(2);
        options = [0 0 0 0 0];
    elseif length(options) == 7,
        plotTypeFlag = options(1);
        visualizeFlag = options(2);
        options = options(3:7);
    else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
result = analyzeImportantInteractions(ODEfctname, steadystate, options, visualizeFlag);
...
% behavior.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [result] = analyzeImportantInteractions(ODEfctname, steadystate, options, visualizeFlag)
% definition of options
if options(1) == 0, omegaRange = 10; else omegaRange = options(1); end
if options(2) == 0, omegaRangePoints = 50; else omegaRangePoints = options(2); end
if options(3) == 0, bisectionThreshold = 1e-14; else bisectionThreshold = options(3); end
if options(4) == 0, criticalLocusTolerance = 1e-5; else criticalLocusTolerance = options(4); end
if options(5) == 0, criticalLocusNrPoints = 10; else criticalLocusNrPoints = options(5); end
