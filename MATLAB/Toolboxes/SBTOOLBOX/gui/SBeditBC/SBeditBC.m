...
% - In the "STATE INFORMATION"-part of the model description only initial
% conditions and optional information is entered. No ODEs.
...
function simulate_Callback(hObject, eventdata, handles)
    ...
    if ~errorFlag,
        ...
        try
            ...
            outputSimulation = SBsimulate(sbm,'ode23s',tspan,SBinitialconditions(sbm));
        catch
        ...
        if ~errorSimulation,
            ...
            handles.SBplotHandle = SBplot(time,datavalues,datanames);
...
function updateICsimulate_Callback(hObject, eventdata, handles)
    ...
    if ~errorFlag,
        ...
        sbm = SBinitialconditions(sbm,lastSimulationValue);
...
function updateIC_Callback(hObject, eventdata, handles)
    ...
    if ~errorFlag,
        % set new initial conditions
        sbm = SBinitialconditions(sbm,steadyStateValues);
        % convert model to text again 
        handles.editText = convertModelToTextBCSB(sbm);  
    end
    % Update handles structure
...
function htmlnotes_Callback(hObject, eventdata, handles)
    ...
    filefullpath = strcat(tempname,'.html');
...
function [helpText] = getHelpText()
    ...
    helpText.events = 'In this view you can enter events that are used to set state variables of the model to certain values when a certain condition is fulfilled. Contrary to SBML here in this toolbox no delay is allowed between the firing of an event and its execution. The syntax for an event is: "eventname=trigger,variable1,value1,...,variablen,valuen %% optional comment". The values can be numerical but also be represented by a formula that is allowed to contain states, parameters, etc. The trigger expression has to have the following simple syntax: "operator(expression1,expression2)", where the operator can be chosen as "ge,gt,le,lt". The two expressions have the same syntax as the values. The event is only fired when the trigger function goes from false to true. The optional comment is not allowed to contain the character "=".';
