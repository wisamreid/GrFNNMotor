...
extern "C" {
 ...
 static void _hoc_LOG2(void);
 static void _hoc_TestITree(void);
 static void _hoc_install(void);
 static void _hoc_testoddometer(void);
...
extern Memb_func* memb_func;
 ...
 "setdata_infot", _hoc_setdata,
 "EXP_infot", _hoc_EXP,
 "LOG2_infot", _hoc_LOG2,
 "TestITree_infot", _hoc_TestITree,
 "install_infot", _hoc_install,
 "testoddometer_infot", _hoc_testoddometer,
 0, 0
};
#define EXP EXP_infot
#define LOG2 LOG2_infot
#define TestITree TestITree_infot
#define _f_LOG2 _f_LOG2_infot
#define _f_EXP _f_EXP_infot
#define testoddometer testoddometer_infot
 extern double EXP( double );
 extern double LOG2( double );
 extern double TestITree( );
 extern double _f_LOG2( double );
 extern double _f_EXP( double );
 extern double testoddometer( );
 /* declare global and static user variables */
#define KTProb KTProb_infot
 double KTProb = 0;
#define MAXLOG2 MAXLOG2_infot
 double MAXLOG2 = 32;
#define MINLOG2 MINLOG2_infot
 double MINLOG2 = 0.001;
#define MAXEXP MAXEXP_infot
 double MAXEXP = 20;
#define MINEXP MINEXP_infot
 double MINEXP = -20;
#define binmax binmax_infot
 double binmax = 0;
#define binmin binmin_infot
 double binmin = 0;
#define beg beg_infot
 double beg = 0;
#define cutoff cutoff_infot
 double cutoff = 0.2;
#define count count_infot
 double count = 0;
#define end end_infot
 double end = 0;
#define installed installed_infot
 double installed = 0;
#define usetable usetable_infot
 double usetable = 1;
#define useslice useslice_infot
 double useslice = 0;
#define verbose verbose_infot
 double verbose = 0.5;
 ...
 "installed_infot", &installed_infot,
 "verbose_infot", &verbose_infot,
 "useslice_infot", &useslice_infot,
 "beg_infot", &beg_infot,
 "end_infot", &end_infot,
 "MINEXP_infot", &MINEXP_infot,
 "MAXEXP_infot", &MAXEXP_infot,
 "MINLOG2_infot", &MINLOG2_infot,
 "MAXLOG2_infot", &MAXLOG2_infot,
 "count_infot", &count_infot,
 "cutoff_infot", &cutoff_infot,
 "binmin_infot", &binmin_infot,
 "binmax_infot", &binmax_infot,
 "KTProb_infot", &KTProb_infot,
 "usetable_infot", &usetable_infot,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 0, 0);
 	...
 	ivoc_help("help ?1 infot /Users/STL/Documents/School/Grad School/Research/Iran Roman/GrFNNMotor/MATLAB/composite/network/x86_64/infot.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
 static double *_t_EXP;
 static double *_t_LOG2;
static int _reset;
...
static int install();
 static double _n_LOG2(double);
 static double _n_EXP(double);
...
double testoddometer (  ) {
   double _ltestoddometer;
...
static void _hoc_testoddometer(void) {
 ...
 static double _mfac_EXP, _tmin_EXP;
 static void _check_EXP();
 static void _check_EXP() {
  static int _maktable=1; int _i, _j, _ix = 0;
  double _xi, _tmax;
  static double _sav_MINEXP;
  static double _sav_MAXEXP;
  if (!usetable) {return;}
  if (_sav_MINEXP != MINEXP) { _maktable = 1;}
  if (_sav_MAXEXP != MAXEXP) { _maktable = 1;}
  if (_maktable) { double _x, _dx; _maktable=0;
   _tmin_EXP =  MINEXP ;
   _tmax =  MAXEXP ;
   _dx = (_tmax - _tmin_EXP)/50000.; _mfac_EXP = 1./_dx;
   for (_i=0, _x=_tmin_EXP; _i < 50001; _x += _dx, _i++) {
    _t_EXP[_i] = _f_EXP(_x);
   }
   _sav_MINEXP = MINEXP;
   _sav_MAXEXP = MAXEXP;
 ...
 double EXP(double _lx){ _check_EXP();
 return _n_EXP(_lx);
 }

 static double _n_EXP(double _lx){ int _i, _j;
 double _xi, _theta;
 if (!usetable) {
 return _f_EXP(_lx); 
}
...
static void _hoc_EXP(void) {
 ...
 static double _mfac_LOG2, _tmin_LOG2;
 static void _check_LOG2();
 static void _check_LOG2() {
  static int _maktable=1; int _i, _j, _ix = 0;
  double _xi, _tmax;
  static double _sav_MINLOG2;
  static double _sav_MAXLOG2;
  if (!usetable) {return;}
  if (_sav_MINLOG2 != MINLOG2) { _maktable = 1;}
  if (_sav_MAXLOG2 != MAXLOG2) { _maktable = 1;}
  if (_maktable) { double _x, _dx; _maktable=0;
   _tmin_LOG2 =  MINLOG2 ;
   _tmax =  MAXLOG2 ;
   _dx = (_tmax - _tmin_LOG2)/50000.; _mfac_LOG2 = 1./_dx;
   for (_i=0, _x=_tmin_LOG2; _i < 50001; _x += _dx, _i++) {
    _t_LOG2[_i] = _f_LOG2(_x);
   }
   _sav_MINLOG2 = MINLOG2;
   _sav_MAXLOG2 = MAXLOG2;
  }
 }

 double LOG2(double _lx){ _check_LOG2();
 return _n_LOG2(_lx);
 }

 static double _n_LOG2(double _lx){ int _i, _j;
 double _xi, _theta;
 if (!usetable) {
 return _f_LOG2(_lx); 
}
...
static void _initlists() {
 int _i; static int _first = 1;
  if (!_first) return;
   _t_EXP = makevector(50001*sizeof(double));
   _t_LOG2 = makevector(50001*sizeof(double));
_first = 0;
}
